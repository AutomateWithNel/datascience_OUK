---
type: moodle-exposure-report
course_code: DSC 201
course_title: Python for Data Science
checked: 2026-06-19
source: Moodle API
---

# Moodle Exposure Report

## Current Scope
Modules 1 to 4 are currently open/exposed and should be used for the active DSC 201 workflow.

## Course Home Page
- Explore the Learning on SOMAS Tutorial (optional) - page
- Announcements - forum
- Class Questions Forum - forum
- DSC 201 Live sessions - Zoom
- Course Resources - folder

## Module 1: Numpy
Status: open

Exposed activities:
- Environment setup video - URL
- Introduction to Numpy - resource
- Recordings 12/3/25-Numpy - URL
- Comprehension Quiz - quiz
- Module 1 Practical Task - quiz
- End of module 1 assessment - assignment
- Join the Conversation - forum
- Test Your Knowledge - quiz
- Reflect on Your Journey - journal

Quiz details exposed:
- Comprehension Quiz: 2 attempts, grade 10
- Module 1 Practical Task: 3 attempts, grade 8
- Test Your Knowledge: 2 attempts, grade 10

## Module 2: Introduction to Pandas
Status: open

Exposed activities:
- Watch and Discover - URL
- Introduction to Pandas - resource
- Recordings 19/03/25-pandas - URL
- Module 2 Mastery quiz - quiz
- Module 2: Practical task - quiz
- Join the Conversation - forum
- Module 2_end quiz - quiz
- Reflect on Your Journey - journal

Quiz details exposed:
- Module 2 Mastery quiz: 2 attempts, grade 5
- Module 2: Practical task: 2 attempts, grade 10
- Module 2_end quiz: 2 attempts, grade 10

## Module 3: Data Loading and File Formats in Pandas
Status: open

Exposed activities:
- Watch and Discover - URL
- Data Loading and file formats - resource
- Recordings-Data Loading - URL
- Module 3 Practical task - VPL
- Join the Conversation - forum
- Module 3 Mastery quiz - quiz
- Reflect on Your Journey - journal

Quiz details exposed:
- Module 3 Mastery quiz: 2 attempts, grade 10, sumgrades 8

Local processing status:
- Moodle learning resource imported into `01 Source Materials`.
- Module note created in `02 Notes`.

## Module 4: Data Cleaning and Preparation
Status: open

Exposed activities:
- Watch and Discover - URL
- Data cleaning notes - resource
- Recordings-Data Cleaning - URL
- Module 4 Mastery quiz - quiz
- Join the Conversation - forum
- Reflect on Your Journey - journal

Quiz details exposed:
- Module 4 Mastery quiz: 2 attempts, grade 10, sumgrades 5

Local processing status:
- Moodle learning resource imported into `01 Source Materials`.
- Module note created in `02 Notes`.

## Later Sections
- Module 5: Data Wrangling - visible but no activities exposed
- Module 6: Visualization and Plotting - visible but no activities exposed
- Module 7: Aggregation and Group Operations - visible but no activities exposed
- Module 8: Exploratory Data Analysis (Deep Dive) - visible but no activities exposed
- Module 9: Handling Time Series Data - visible but no activities exposed
- Module 10: Introduction to modelling libraries in python - visible but no activities exposed

## Final Examinations
- Course/Lecturer Evaluation - questionnaire
- Zoom Troubleshooting Link - Zoom
- DSC 201 JULY 2026 EXAMINATION - quiz

Do not use the final examination quiz in the normal module-note workflow.
